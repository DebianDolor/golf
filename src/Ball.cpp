...
Ball::Ball(Coordinate position, SDL_Texture* texture, SDL_Texture* pointTex, SDL_Texture* powerMTexFG, 
    SDL_Texture* powerMTexBG, int index):Entity(position, texture)
{
    this->index = index;
    points.push_back(Entity(Coordinate(-64, -64), pointTex));
    powerBar.push_back(Entity(Coordinate(-64, -64), powerMTexBG));
    powerBar.push_back(Entity(Coordinate(-64, -64), powerMTexFG));
}
...
void Ball::setWin(bool win)
{
    this->win = win;
}

void Ball::update(double deltaTime, bool mouseDown, bool mousePressed, std::vector<Wall> walls, 
